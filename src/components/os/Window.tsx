import React, { ReactNode, HTMLAttributes, useEffect, useRef, useState } from 'react';
...
export interface WindowProps extends HTMLAttributes<HTMLDivElement> {
    ...
    left?: number;
    windowTitle?: string;
    title?: string; // Alias para windowTitle para compatibilidad
...
const Window: React.FC<WindowProps> = (props) => {
    ...
    const title = props.title || props.windowTitle || '';
    const initialSize = props.initialSize || { width: props.width || 300, height: props.height || 200 };
    ...
    const [width, setWidth] = useState(props.width || initialSize.width);
    const [height, setHeight] = useState(props.height || initialSize.height);
    ...
    useEffect(() => {
        if (props.onWidthChange && contentWidth !== undefined) {
            props.onWidthChange(contentWidth);
        }
    }, [props.onWidthChange, contentWidth]); // eslint-disable-line

    useEffect(() => {
        if (props.onHeightChange && contentHeight !== undefined) {
    ...
    const onWindowInteract = () => {
        props.onInteract && props.onInteract();
        setWindowActive(true);
        lastClickInside.current = true;
    };

    // Determinar si usar el estilo nuevo o el antiguo
    const useNewStyle = props.className && props.className.includes('window--clippy');
    
    if (useNewStyle) {
        // Nuevo estilo con CSS
            ...
            >
                <div className="title-bar">
                    ...
                    <div className="title-bar-controls">
                        <button aria-label="Minimize" onClick={props.minimizeWindow} />
                        <button aria-label="Maximize" onClick={maximize} />
                        <button aria-label="Close" onClick={props.closeWindow} />
                    </div>
                </div>
